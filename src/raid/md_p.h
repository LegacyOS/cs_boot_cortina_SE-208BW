...
#define BLOCK_SIZE (1<<BLOCK_SIZE_BITS)		// 1024 byte
...
#define SYSTEM_DEV	0x1
#define DATA_DEV	0x2


#define MD_DEV_WORKABLE	0x01
#define MD_DEV_FAIL	0x02

#define MAX_MD_DEV		4
#define MAX_RAID_DISK		4
typedef struct md_dev_info_s {
	u32		flag;		// system or data
	u8		status;		// workable
	IDE_DISK_T	virt_disk;
	IDE_PART_T	*disk[MAX_RAID_DISK];
	u8		disk_no;
	u32		uuid[4];
	u32		chunk_size;
	...
	int		level;
	u64		array_size;	// block no.(1024byte unit)
} md_dev_info_t;
