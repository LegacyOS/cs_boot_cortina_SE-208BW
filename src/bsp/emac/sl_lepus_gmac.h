...
typedef union
{
	...
	{
#if (BIG_ENDIAN==1)
		unsigned int	reserved	: 9;	// bit 31:19 	Tx Flag, Reserved
...
#define TX_DESC_SOF_BIT			2
#define TX_DESC_EOF_BIT			1
...
#define RX_DESC_SOF_BIT			2
#define RX_DESC_EOF_BIT			1
...
typedef union
{
	...
	{
#if (BIG_ENDIAN==1)
		unsigned int reserved       : 16;	// bit 31-16	
...
typedef union
{
	...
	struct bit1_002c
	{
		...
		unsigned int reserved		:  1;	/* PHY interface mode in 10M-bps */
...
typedef struct {
	...
	unsigned short		hw_rptr;
	GMAC_TXDESC_T		*tx_curr_desc;
	GMAC_TXDESC_T		*finished_curr_desc;
...
typedef struct gmac_private {
	void 				*ndt;		//point to  this cyg_netdevtab_entry
	//struct eth_drv_sc	*sc;
	unsigned int		existed;
	unsigned int		port_id;	// 0 or 1
	unsigned int		base_addr;
	unsigned int		dma_base_addr;
	unsigned char		mac_addr1[ETHER_ADDR_LEN];
	unsigned char		mac_addr2[ETHER_ADDR_LEN];
	...
	//cyg_interrupt  		intr_object;
	// cyg_drv_mutex_t 	tx_mutex;
	void 				*tx_mutex;
	...
	struct ether_drv_stats ifStatics;
	// void				*ifStatics;
	unsigned long		txDerr_cnt[2];
	unsigned long		txPerr_cnt[2];
	unsigned long		RxDerr_cnt[2];
	unsigned long		RxPerr_cnt[2];
...
typedef struct {
	...
	unsigned int		rx_curr_desc;
	struct sk_buff		*curr_rx_skb;
	unsigned long       rx_q_cnt;
	unsigned long		rx_good;
	unsigned long		rx_count;
	unsigned long		rx_crc_errors;
	unsigned long		intr_full_cnt;
...
typedef struct {
	INTR_QHDR_T			*desc_base;
	unsigned long		desc_num;
	NONTOE_QHDR_T		*qhdr;
	DMA_RWPTR_T			rwptr;
	INTR_QHDR_T			*rx_curr_desc;
	unsigned long       rx_q_cnt;
	unsigned long		intr_full_cnt;
	unsigned long		intr_cnt;
} INTRQ_INFO_T;
...
typedef struct toe_private {
	...
	dma_addr_t		toe_desc_base_dma;
	dma_addr_t		class_desc_base_dma;
	dma_addr_t		intr_desc_base_dma;
	INTR_REG0_T		intr0_select;	// 0: int 0, 1: int1
	INTR_REG1_T		intr1_select;	// 0: int 0, 1: int1
	INTR_REG2_T		intr2_select;	// 0: int 0, 1: int1
	INTR_REG3_T		intr3_select;	// 0: int 0, 1: int1
	INTR_REG4_T		intr4_select;	// 0: int 0, 1: int1
	INTR_REG0_T		intr0_device;	// 0: eth0,	1:eth1
	INTR_REG1_T		intr1_device;	// 0: eth0,	1:eth1
	INTR_REG2_T		intr2_device;	// 0: eth0,	1:eth1
	INTR_REG3_T		intr3_device;	// 0: eth0,	1:eth1
	INTR_REG4_T		intr4_device;	// 0: eth0,	1:eth1
	// struct sk_buff	*rx_skb[TX_DESC_NUM];
	GMAC_INFO_T		gmac;//gmac[GMAC_NUM];
	CLASSQ_INFO_T	classq[TOE_CLASS_QUEUE_MAX];
	INTRQ_INFO_T	intrq[TOE_INTR_QUEUE_MAX];
	TOEQ_INFO_T		toeq[TOE_TOE_QUEUE_MAX];
	void			(*gmac_rcv_handler)(struct sk_buff *, int);
} TOE_INFO_T;
