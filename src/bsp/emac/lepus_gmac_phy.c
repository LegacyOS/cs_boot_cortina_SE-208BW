	...
	for(i = 0; i < 2; i++)
	  {
	    ...
	    reg_val=(mii_read(i ,0x02) << 16) + mii_read(i ,0x03);
...
void astelMiiRead(char argc, char *argv[])
{
	unsigned int ret;
	...
	if(argc != 3)
	  {
		printf("Command Error!! check Command: miiread phy_addr reg_addr \n");
		return;
	  }

	phyad = ascii_hex(argv[1]);
	regad = ascii_hex(argv[2]);

	ret = mii_read(phyad, regad);
	printf("PHY%d: 0x%x=0x%x\n",phyad, regad, ret);
}
...
void mii_write(unsigned char phyad,unsigned char regad,unsigned int value)
{
	...
	printf("    %s: phy_addr=0x%x reg_addr=0x%x value=0x%04x \n", __func__, phyad,regad, value);
...
void astelMiiWrite(char argc, char *argv[])
{
	unsigned char phyad, regad;
	unsigned int value;
	...
	if(argc != 4)
	  {
		printf("Command Error!! check Command: miiwrite phy_addr reg_addr data \n");
		return;
	  }

	phyad = ascii_hex(argv[1]);
	regad = ascii_hex(argv[2]);
	value = ascii_hex(argv[3]);
